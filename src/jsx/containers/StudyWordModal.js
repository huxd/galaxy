...
class StudyWordModal extends React.Component {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        state.meanings = word.meanings;
        variant.origin = word.name;
        for(let meaningItem of state.meanings) {
            for(let key in variant) {
                if(variant[key] != 'none') {
                    meaningItem.example = meaningItem.example.replace(new RegExp(variant[key],'igm'), '<span class="text-danger"><b>$&</b></span>');
                }
            }
        }
